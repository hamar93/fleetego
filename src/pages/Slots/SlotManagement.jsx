    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Időkapu Kezelés</h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400">ETA alapú felrakó / lerakó időpontok és várakozási idők.</p>
                </div>
                <button
                    onClick={fetchSlots}
                ...
                <>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-gray-50 dark:bg-slate-700/50 border-b border-gray-200 dark:border-slate-700 text-xs uppercase tracking-wider text-gray-500 dark:text-gray-400">
                                        <th className="p-4 font-semibold">Típus</th>
                                        <th className="p-4 font-semibold">Cím</th>
                                        <th className="p-4 font-semibold">Dátum</th>
                                        <th className="p-4 font-semibold">Időablak (Start - End)</th>
                                        <th className="p-4 font-semibold">Jármű / Rendelés</th>
                                        <th className="p-4 font-semibold">Várakozási Idő</th>
                                    ...
                                    {slots.length === 0 ? (
                                        <tr>
                                            <td colSpan="8" className="p-8 text-center text-gray-500">
                                                {loading ? 'Betöltés...' : 'Nincsenek aktív időkapufoglalások.'}
                                            </td>
                                        </tr>
                                    ) : (
                                        slots.map(slot => (
                                            <tr key={slot.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/20 transition-colors">
                                                <td className="p-4 font-medium text-gray-900 dark:text-white">
                                                    {slot.stop_type === 'LOADING' ? (
                                                        <span className="flex items-center gap-1.5"><i className="fas fa-box-open text-blue-500"></i> Felrakás</span>
                                                    ) : (
                                                        <span className="flex items-center gap-1.5"><i className="fas fa-box text-green-500"></i> Lerakás</span>
                                                    )}
                                                </td>
                                                ...
                                                <td className="p-4">
                                                    {slot.status === 'DELAYED' ? (
                                                        <span className="text-red-600 font-medium flex items-center gap-1"><i className="fas fa-clock"></i> ETA Csúszás</span>
                                                ...
                                                <td className="p-4 text-right">
                                                    <button className="text-blue-600 hover:text-blue-800 p-2 rounded-lg hover:bg-blue-50 transition-colors" title="Részletek">
                    ...
                    <div className="mt-6 flex justify-end">
                        <button className="px-5 py-2.5 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition shadow-sm flex items-center gap-2">
                            <i className="fas fa-plus"></i> Új Foglalás (Manuális)
                        </button>
                    </div>
                </>
            )}
            ...
            {activeTab === 'report' && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="p-6 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700">
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Átlagos Várakozás</h3>
                        <div className="text-4xl font-bold text-blue-600 dark:text-blue-400">1ó 15p</div>
                        <p className="text-sm text-gray-500 mt-2">Az elmúlt 30 napban csökkent 15 perccel.</p>
                    </div>
                    <div className="p-6 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700">
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Legproblémásabb Hely (Top 1)</h3>
                        <div className="text-xl font-bold text-red-600 dark:text-red-400">DE-80331 München</div>
                        <p className="text-sm text-gray-500 mt-2">Átlagos várakozás: 3ó 45p</p>
                    </div>
                    <div className="p-6 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700">
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Időkapu Betartás</h3>
                        <div className="text-4xl font-bold text-green-600 dark:text-green-400">85%</div>
                        <p className="text-sm text-gray-500 mt-2">A járművek az ETA alapján pontosan érkeznek a kapuhoz.</p>
                    </div>
                    <div className="col-span-1 md:col-span-3 p-6 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 h-64 flex items-center justify-center">
                        <p className="text-gray-500 flex flex-col items-center">
                            <i className="fas fa-chart-bar text-4xl mb-4 text-gray-300"></i>
                            Később ide érkezik a részletes diagram a "Yard Waiting Killer" funkcióból kinyert pontos ETA és valós mozgás adatok alapján.
                        </p>
                    </div>
                </div>
            )}
