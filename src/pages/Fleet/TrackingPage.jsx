...
const TrackingPage = () => {
    ...
    return (
        <div className="p-6 h-[calc(100vh-80px)] flex flex-col">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">📍 {t('tracking_page.title')}</h1>

            <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 overflow-hidden">
                {/* List View */}
                <div className="bg-white dark:bg-[#1e293b] rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col">
                    <div className="p-4 border-b border-gray-100 dark:border-gray-700 font-semibold text-gray-700 dark:text-gray-200">
                        {t('tracking_page.vehicle_list')} ({vehicles.length})
                    </div>
                                ...
                                <div key={v.id} className="p-3 rounded-lg border border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50 cursor-pointer transition-colors">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div className={`px-2 py-0.5 rounded text-xs font-medium
                                            ...
                                            {v.status === 'active' ? t('tracking_page.status_active') : v.status === 'idle' ? t('tracking_page.status_idle') : v.status}
                                        </div>
                                    </div>
                                    <div className="mt-2 text-sm text-gray-600 dark:text-gray-400 flex items-center gap-2">
                                        <i className="fas fa-map-marker-alt text-red-500"></i>
                                        {v.location || t('tracking_page.unknown_location')}
                                    </div>
                ...
                {/* Map Placeholder */}
                <div className="lg:col-span-2 bg-gray-100 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 flex flex-col items-center justify-center text-gray-400 relative overflow-hidden">
                    <iframe
                        width="100%"
                        height="100%"
                        frameBorder="0"
                    ...
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none bg-black/5">
                        <div className="bg-white/90 dark:bg-black/80 px-4 py-2 rounded-lg shadow-lg text-sm font-medium text-gray-800 dark:text-gray-200 backdrop-blur-sm">
                            {t('tracking_page.gps_coming_soon')}
                        </div>
                    </div>
                </div>
            </div>
