...
const OrderDetails = () => {
    ...
    const getStatusLabel = (status) => {
        const labels = {
            ...
            assigned: 'Kiosztva',
            pickup: 'Felvételen',
            loading_started: 'Felrakás...',
            loading_completed: 'Felrakva',
            transit: 'Úton',
            unloading_started: 'Lerakás...',
            unloading_completed: 'Lerakva',
            delivered: 'Kész',
            cancelled: 'Törölve'
        };
        ...
        <div className="p-6 max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start mb-6 gap-4">
                ...
                <div className="flex gap-2">
                    ...
                    {order.status === 'pickup' && (
                        <button
                            onClick={() => updateStatus('loading_started')}
                            className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg font-medium shadow-sm transition-colors"
                        >
                            Felrakás kezdése
                        </button>
                    )}
                    {order.status === 'loading_started' && (
                        <button
                            onClick={() => updateStatus('loading_completed')}
                            className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg font-medium shadow-sm transition-colors"
                        >
                            Felrakás kész
                        </button>
                    )}
                    {order.status === 'loading_completed' && (
                        <button
                    ...
                    {order.status === 'transit' && (
                        <button
                            onClick={() => updateStatus('unloading_started')}
                            className="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg font-medium shadow-sm transition-colors"
                        >
                            Lerakás kezdése
                        </button>
                    )}
                    {order.status === 'unloading_started' && (
                        <button
                            onClick={() => updateStatus('unloading_completed')}
                            className="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg font-medium shadow-sm transition-colors"
                        >
                            Lerakás kész
                        </button>
                    )}
                    {order.status === 'unloading_completed' && (
                        <button
            ...
            {/* Alert Banner (Dispatch Copilot) */}
            {order.alert_status && order.alert_status !== 'OK' && (
                <div className={`mb-6 p-4 rounded-xl border-l-4 shadow-sm flex items-start gap-4 animate-pulse-slow
                    ${order.alert_status === 'CRITICAL' ? 'bg-red-50 dark:bg-red-900/20 border-red-500 text-red-800 dark:text-red-300' : 'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-500 text-yellow-800 dark:text-yellow-300'}`}>
                    <div className="text-2xl">
                        {order.alert_status === 'CRITICAL' ? '🚨' : '⚠️'}
                    </div>
                    <div>
                        <h3 className="font-bold text-lg">
                            {order.alert_status === 'CRITICAL' ? 'Kritikus Figyelmeztetés' : 'Figyelmeztetés'}
                        </h3>
                        <p className="font-medium">
                            {order.alert_message}
                        </p>
                        <p className="text-sm opacity-80 mt-1">
                            A rendszer beavatkozást vagy ellenőrzést javasol.
                        </p>
                    </div>
                </div>
            )}
            ...
            <div className="mb-8 overflow-x-auto pb-4">
                <div className="min-w-[900px] flex justify-between items-center relative">
                    ...
                    {['created', 'assigned', 'pickup', 'loading_started', 'loading_completed', 'transit', 'unloading_started', 'unloading_completed', 'delivered'].map((step, index) => {
                        const isCompleted = getStatusStep(order.status) > index;
                        const isCurrent = getStatusStep(order.status) === index;

                        return (
                            <div key={step} className="flex flex-col items-center gap-2 bg-white dark:bg-[#0f172a] px-2 py-1 z-10">
                                ...
                                <span className={`text-[10px] uppercase tracking-wider font-bold text-center w-20 ${isCurrent || isCompleted ? 'text-gray-900 dark:text-white' : 'text-gray-400'}`}>
                                    {getStatusLabel(step)}
                                </span>
                            </div>
                        );
            ...
            <div className="flex border-b border-gray-200 dark:border-gray-700 mb-6">
                ...
                <button
                    ...
                    className={`px-6 py-3 font-medium text-sm transition-colors border-b-2 ${activeTab === 'documents' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                >
                    Dokumentumok
                </button>
                <button
                    onClick={() => setActiveTab('matching')}
                    className={`px-6 py-3 font-medium text-sm transition-colors border-b-2 ${activeTab === 'matching' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                >
                    Matching
                </button>
            </div>
